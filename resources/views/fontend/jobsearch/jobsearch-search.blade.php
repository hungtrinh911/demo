        ...
        <div class="top-employer-wrap">
          <div class="col-xl-12">
            ...
            <div class="top-employer-list">
              @foreach($hot_employer as $employer)
              <div class="item"><a class="top-employer-img" href="{{action('Fontend\JobSearchController@jobsearchdetail',$employer->id)}}"><img class="img-fluid" src="{{asset("$employer->featured_img")}}" alt=""/></a><a class="top-employer-name" href="{{action('Fontend\JobSearchController@jobsearchdetail',$employer->id)}}">{{$employer->excerpt}}</a></div>
              @endforeach
            </div>
          </div>
        </div>
            ...
            <div class="col-xl-3">
              <div class="ads-list">
                ...
                <div class="ads-list-item">
                   @if($randomjob != null)
                    @foreach($randomjob as $rd)
                    <a href="{{action('Fontend\JobSearchController@jobsearchdetail',$rd->id)}}"><img class="img-fluid" src="{{asset("$rd->featured_img")}}" alt=""/></a>
                    @endforeach
                  @endif
              </div>
            </div>
    ...
    container.pagination({
      ...
      callback: function(response, pagination) {
        ...
        $.each(response, function (index, item) {
          id = item.id;
          dataHtml += '<div class="item"><div class="job-info"><div class="job-img"><a href="#"><img class="img-fluid" src="'+item.featured_img+'" alt=""/></a></div>  <div class="job-desc"><h3><a href="{{action('Fontend\JobSearchController@jobsearchdetail',array('id'=>'')) }}/' + item.id + '">'+item.title+'</a></h3><p class="company">'+item.excerpt+'</p> <div class="desciption"><div style=" overflow: hidden; text-overflow: ellipsis; white-space: nowrap; ">'+item.description+'</div></div></div></div><div class="job-action"><p class="times">'+item.created_at+'</p></div></div>';
