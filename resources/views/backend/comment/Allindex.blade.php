               ...
               <div class="col-sm-12">
                <div class="card-box">
                    ...
                    <table id="grid" class="table-bordered "
                           ...
                           data-url="{{ action('Api\CommentController@grid') }}"
    ...
    <script>
        ...
        function gridAction(value, row) {
            ...
            @if(Auth::user()->can('edit-news'))
                actions +='<a href="{{ action('Backend\CommentController@showEditAllCommentForm', array('id'=>'')) }}/' + row.id + '" class="on-default edit-row text-primary"><i class="fa fa-pencil"></i></a> ';
            @endif
          
            @if(Auth::user()->can('delete-news'))
                actions += '<a href="javascript:;" class="on-default remove-row text-danger grid-btn-delete" data-placement="top" title="" data-original-title="Delete" data-toggle="modal" data-target=".modal-delete" data-href="{{  action('Backend\CommentController@destroy', array('id'=>'')) }}/' + row.id + '" class="on-default edit-row text-primary"><i data-id="' + row.id + '" class="fa fa-trash-o grid-btn-delete"></i></a> ';
            @endif
