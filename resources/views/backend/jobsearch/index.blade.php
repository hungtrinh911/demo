        ...
        <div class="row">
            <div class="col-sm-12">
                ...
                <div class="btn-group pull-right m-t-15">
                    <a href="{{ action('Backend\JobSearchController@showAddForm') }}" class="btn btn-default">
                ...
                <ol class="breadcrumb">
                    <li class="breadcrumb-item">
                        <a href="{{ action('Backend\JobSearchController@index') }}">Job Search</a></li>
            ...
            <div class="col-sm-12">
                <div class="card-box">
                    ...
                    <table id="grid" class="table-bordered "
                           ...
                           data-url="{{ action('Api\JobSearchController@grid') }}"
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.form-delete').on('submit', function () {
                ...
                $.ajax({
                    url: '{{ action('Api\JobSearchController@delete') }}', type: 'POST', dataType: 'JSON',
                    data: $(this).serialize(),
        ...
        function gridAction(value, row) {
            ...
            @if(Auth::user()->can('edit-news'))
                actions += '<a href="{{ action('Backend\JobSearchController@showEditForm', array('id'=>'')) }}/' + row.id + '" class="on-default edit-row text-primary"><i class="fa fa-pencil"></i></a> ';
            @endif
             @if(Auth::user()->can('delete-news'))
                actions += '<a href="javascript:;" class="on-default remove-row text-danger grid-btn-delete" data-placement="top" title="" data-original-title="Delete" data-toggle="modal" data-target=".modal-delete" data-href="{{  action('Backend\JobSearchController@destroy', array('id'=>'')) }}/' + row.id + '" class="on-default edit-row text-primary"><i data-id="' + row.id + '" class="fa fa-trash-o grid-btn-delete"></i></a> ';
            @endif
