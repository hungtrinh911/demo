        ...
        <div class="row">
            <div class="col-sm-12">
                ...
                <div class="btn-group pull-right m-t-15">
                    <a href="{{ action('Backend\TrainingCourseController@showAddForm') }}" class="btn btn-default">
            ...
            <div class="col-sm-12">
                <div class="card-box">
                    ...
                    <table id="grid" class="table-bordered "
                           ...
                           data-url="{{ action('Api\TrainingCourseController@grid') }}"
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.form-delete').on('submit', function () {
                ...
                $.ajax({
                    url: '{{ action('Api\TrainingCourseController@delete') }}', type: 'POST', dataType: 'JSON',
                    data: $(this).serialize(),
        ...
        function gridAction(value, row) {
            var actions = '';
            @if(Auth::user()->can('edit-trainingcourse'))
                actions += '<a href="{{ action('Backend\TrainingCourseController@showEditForm', array('id'=>'')) }}/' + row.id + '" class="on-default edit-row text-primary"><i class="fa fa-pencil"></i></a> ';
             @endif
      
             @if(Auth::user()->can('delete-trainingcourse'))
                actions += '<a href="javascript:;" class="on-default remove-row text-danger grid-btn-delete" data-placement="top" title="" data-original-title="Delete" data-toggle="modal" data-target=".modal-delete" data-href="{{ action('Backend\TrainingCourseController@destroy', array('id'=>'')) }}/' + row.id + '" class="on-default edit-row text-primary"><i data-id="' + row.id + '" class="fa fa-trash-o grid-btn-delete"></i></a> ';
            @endif
                return actions;
        }
    </script>
