...
@section('title', 'Chuyên mục TrainingCourse')
@section('css')
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="btn-group pull-right m-t-15">
                    <a href="{{ action('Backend\TrainingCourseController@category') }}" class="btn btn-default">
                ...
                <ol class="breadcrumb">
                    <li class="breadcrumb-item">
                        <a href="{{ action('Backend\TrainingCourseController@index') }}">Training Course</a></li>
            ...
            <div class="col-lg-4 col-form">
                <div class="card-box">
                    ...
                    <form method="post" class="form-add d-none"
                          action="{{ action('Backend\TrainingCourseController@addCategory') }}">
                        {{ csrf_field() }}
                        ...
                        <div class="form-group">
                            ...
                            <select id="parent_id" name="parent_id" class="selectpicker show-tick"
                                ...
                                @foreach(\App\NewsCategory::treeTrainingCourse('', true) as $item)
                        ...
                        <div class="form-group">
                            ...
                            <select id="locale_source_id" name="locale_source_id" class="selectpicker show-tick"
                                ...
                                @foreach(\App\NewsCategory::orphanListTrainingCourse() as $item)
                                    <option value="{{ $item->id }}">{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <form method="post" class="form-edit d-none"
                          action="{{ action('Backend\TrainingCourseController@editCategory', array('id', $term->id)) }}">
                        {{ csrf_field() }}
                        <input type="hidden" id="id" name="id" value="{{ $term->id }}">
                        ...
                        <div class="form-group">
                            ...
                            <select id="parent_id" name="parent_id" class="selectpicker show-tick"
                                    data-style="btn-white">
                                @foreach(\App\NewsCategory::treeTrainingCourse('', true, '---', request()->route('id')) as $item)
            ...
            <div class="col-lg-8 col-grid">
                <div class="card-box">
                    ...
                    <table class="table table-hover">
                        ...
                        @foreach(\App\NewsCategory::treeTrainingCourse('', false) as $item)
                            <tr>
                                {{--<th scope="row">1</th>--}}
                                <td>{{ $item->name }}</td>
                                <td>{{ $item->slug }}</td>
                                <td class="actions">
                                    @if (Auth::user()->can('edit-news_category'))
                                        <a href="{{ action('Backend\TrainingCourseController@showEditCategoryForm', array('id'=>$item->id)) }}"
                                    ...
                                    @if (Auth::user()->can('delete-news_category'))
                                        <a href="javascript:;"
                                           ...
                                           data-href="{{ action('Backend\TrainingCourseController@deleteCategory', array('id'=>$item->id)) }}">
