            ...
            <div class="col-sm-12">
                ...
                <ol class="breadcrumb">
                    <li class="breadcrumb-item">
                        <a href="{{ action('Backend\NewsController@index') }}">Tin tức</a></li>
            ...
            <div class="col-sm-12">
                <div class="card-box">
                    ...
                    <table id="grid" class="table-bordered "
                           ...
                           data-url="{{ action('Api\NewsController@grid') }}"
...
@section('javascript')
 ...
 //console.log( ids);
            ...
            $('.form-delete').on('submit', function () {
                ...
                $.ajax({
                    url: '{{ action('Backend\NewsController@deleteNew',array('id'=>'')) }}', type: 'POST', dataType: 'JSON',
                    data: $(this).serialize(),
        ...
        function gridAction(value, row) {
            ...
            @if(Auth::user()->can('edit-news'))
                actions += '<a href="{{ action('Backend\NewsController@showEditForm', array('id'=>'')) }}/' + row.id + '" class="on-default edit-row text-primary"><i class="fa fa-pencil"></i></a> ';
            @endif
             @if(Auth::user()->can('delete-role'))
                actions += '<a href="javascript:;" class="on-default remove-row text-danger grid-btn-delete" data-placement="top" title="" data-original-title="Delete" data-toggle="modal" data-target=".modal-delete" data-href="{{ action('Backend\NewsController@deleteNew', array('id'=>'')) }}/' + row.id + '" class="on-default edit-row text-primary"><i data-id="' + row.id + '" class="fa fa-trash-o grid-btn-delete"></i></a> ';
            @endif
                return actions;
