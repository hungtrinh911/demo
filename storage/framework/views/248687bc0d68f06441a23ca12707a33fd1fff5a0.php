               ...
               <div class="col-sm-12">
                <div class="card-box">
                    ...
                    <table id="grid" class="table-bordered "
                           ...
                           data-url="<?php echo e(action('Api\TourGuideController@grid')); ?>"
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.form-delete').on('submit', function () {
                ...
                $.ajax({
                    url: '<?php echo e(action('Api\TourGuideController@destroy')); ?>', type: 'POST', dataType: 'JSON',
                    data: $(this).serialize(),
        ...
        function gridAction(value, row) {
            var actions = '';
              <?php if(Auth::user()->can('edit-news')): ?>
                actions += '<a href="<?php echo e(action('Backend\TourGuideController@show_skill', array('id'=>''))); ?>/' + row.id + '" class="on-default edit-row text-primary"><i class="fa fa-info-circle "></i></a> ';
            <?php endif; ?>
            <?php if(Auth::user()->can('edit-news')): ?>
                actions +='<a href="<?php echo e(action('Backend\TourGuideController@edit', array('id'=>''))); ?>/' + row.id + '" class="on-default edit-row text-primary"><i class="fa fa-pencil"></i></a> ';
            <?php endif; ?>
          
            <?php if(Auth::user()->can('delete-news')): ?>
                actions += '<a href="javascript:;" class="on-default remove-row text-danger grid-btn-delete" data-placement="top" title="" data-original-title="Delete" data-toggle="modal" data-target=".modal-delete" data-href="<?php echo e(action('Backend\TourGuideController@destroy', array('id'=>''))); ?>/' + row.id + '" class="on-default edit-row text-primary"><i data-id="' + row.id + '" class="fa fa-trash-o grid-btn-delete"></i></a> ';
            <?php endif; ?>
